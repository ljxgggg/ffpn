...
class RBBlockffem(nn.Module):
    def __init__(self, in_cs, out_c, current, shallow, sf=2, mode="nearest"):
        ...
        super(RBBlockffem, self).__init__()
        ...
        self.ffem = FFEM(c1=in_cs[0], c2=in_cs[0])

    # x = [current layer, shallow layer, deep layer]
    def forward(self, x):
        x1 = self.cv1(self.ffem(x[0]))
        # x1 = self.cv1(x[0])
        x2 = self.cv2(x[1])
...
class FFEM(nn.Module):
    ...
    # 24 [512(ch_in), para:512(ch_out), 1(k), 1(s)]
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1):  # ch_in, ch_out, kernel, stride, padding, groups
        super(FFEM, self).__init__()
        # self.ComplexConv = ComplexConv2d(c1, c2)
        self.conv2d33 = nn.Conv2d(c1, c2, 3, s, autopad(3, 1), groups=g, bias=False)
        self.conv2d34 = nn.Conv2d(c1, c2, 3, s, autopad(3, 1), groups=g, bias=False)
        self.smu13 = SMU()

    # train
    # def forward(self, x):
    #     y = x
    #     y = fft.rfft2(y.to(float), dim=(-2, -1))
    #     y_real = y.real
    #     y_imag = y.imag
    #     # y_real1 = torch.abs(y_real).to(x.dtype)
    #     # y_imag1 = torch.abs(y_imag).to(x.dtype)
    #     # y_real1, y_imag1 = self.ComplexConv(y_real1, y_imag1)
    #     # y_real1, y_imag1 = self.smu11(y_real1), self.smu12(y_imag1)
    #     y_real1 = torch.abs(y_real).to(x.dtype)
    #     y_imag1 = torch.abs(y_imag).to(x.dtype)
    #     y = torch.complex(y_real1.to(float), y_imag1.to(float))
    #     # y_real1 = torch.abs(y_real)
    #     # y_imag1 = torch.abs(y_imag)
    #     # y = torch.complex(y_real1, y_imag1)
    #     # y = torch.complex(y_real, y_imag)
    #     y = fft.irfft2(y, dim=(-2, -1))
    #     y = torch.where(torch.isnan(y), torch.full_like(y, 0), y)
    #     y = torch.where(torch.isinf(y), torch.full_like(y, 0), y)
    #     # y = y.half()
    #     y = y.float()
    #     z = x
    #     z = self.conv2d34(self.smu13(self.conv2d33(z)))
    #     return (x+y+z).to(x.dtype)
    #     # return (x+y+z)
...
class SMU(nn.Module):
    def __init__(self, alpha=0.01):
        ...
        self.alpha = alpha
        # initialize mu
        self.mu = torch.nn.Parameter(torch.tensor(2.5))
...
#                  kernel_sizes=1, stride=1, padding=0, dilation=1, groups=1, bias=True):
...
#         return self.conv_real(input_real) - self.conv_imag(input_imag), self.conv_imag(input_real) + self.conv_real(
#             input_imag)
...
class RBBlockB(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.cv2(x[1])
        # Upsample: (b, c, w, h) => (b, c, w * sf, h * sf)
        x3 = self.up1(self.cv3(x[2]))
        return self.out_cv(torch.cat((x1, x2, x3), dim=1))


# RB Block with ReOrg for shallow layer
class RBBlockC(nn.Module):
    def __init__(self, in_cs, out_c, current, shallow, sf=2, mode="nearest"):
        # current/shallow/deep = [Conv size, kernel size, stride]
        super(RBBlockC, self).__init__()
        self.up1 = nn.Upsample(size=None, scale_factor=sf, mode=mode)  # for deep layer (i+1)-th
        self.reorg = ReOrg()  # for shallow layer (i-1)-th
        self.cv1 = Conv(in_cs[0], *current)  # for current layer i-th
        self.cv2 = Conv(in_cs[1], *shallow)  # for shallow layer (i-1)-th
        self.out_cv = Conv(current[0] + shallow[0] + in_cs[2], out_c, 1, 1)
